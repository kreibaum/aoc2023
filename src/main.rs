...
use std::{cmp::{min, max}, collections::HashMap, result};
...
impl AdventMap {
    ...
    fn map_range(&self, interval: (u128, u128)) -> Vec<(u128, u128)> {
        ...
        for section in &self.map_sections {
            let (left_interval, intersecting_interval, new_remaining_interval) = mapOneSection(section, remaining_interval);
...
fn mapOneSection(
        ...
        && start <= section.source_range_start + section.source_range_length -1
    {
        ...
        let right = min(start + length - 1, section.source_range_start + section.source_range_length - 1);
        Some((
            left,
            right - left + 1,
        ))
    ...
    let new_remaining_interval = if start + length - 1 > section.source_range_start + section.source_range_length - 1 {
        let left = max(start, section.source_range_start + section.source_range_length);
