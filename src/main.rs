...
fn main() {
    ...
    for line in input.lines() {
        ...
        all_hands.push(Hand::new(hand.to_string(), bid));
    }
    ...
    println!("Total winning: {}", total_winning); // 249638405
...
impl Hand {
    fn new(cards: String, bid: u128) -> Self {
        let hand_type = classify_hand_type(cards.clone());
        let integer_proxy = Self::hand_to_integer_proxy(cards.clone(), hand_type);
        Self {
            cards,
            bid,
            hand_type,
    ...
    fn hand_to_integer_proxy(cards: String, hand_type: HandType) -> u64 {
        ...
        hand_type as u64 * 2_u64.pow(20)
            + card_to_ordinal(chars[0]) as u64 * 2_u64.pow(16)
            + card_to_ordinal(chars[1]) as u64 * 2_u64.pow(12)
            + card_to_ordinal(chars[2]) as u64 * 2_u64.pow(8)
            + card_to_ordinal(chars[3]) as u64 * 2_u64.pow(4)
            + card_to_ordinal(chars[4]) as u64
    }
}
...
fn card_to_ordinal(card: char) -> u8 {
    match card {
        '2' => 2,
        '3' => 3,
        '4' => 4,
...
fn classify_hand_type(cards: String) -> HandType {
    let mut count_by_ordinal: [u8; 15] = [0; 15];
    for card in cards.chars() {
        count_by_ordinal[card_to_ordinal(card) as usize] += 1;
    }
